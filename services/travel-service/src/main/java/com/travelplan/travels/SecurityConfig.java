        ...
        .authorizeHttpRequests(
            auth ->
                auth.requestMatchers(
                    ...
                    .permitAll()
                    .requestMatchers(HttpMethod.GET, "/api/travels/**")
                    .authenticated()
                    .requestMatchers("/api/travels/**")
                    .hasAnyRole("ADMIN", "TRAVEL_MANAGER")
                    .anyRequest()
                    .denyAll())
        .oauth2ResourceServer(
            resource -> resource.jwt(config -> config.jwtAuthenticationConverter(jwt)))
        .build();
  }
}
