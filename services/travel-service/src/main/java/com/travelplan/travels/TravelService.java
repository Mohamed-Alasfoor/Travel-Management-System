...
public class TravelService {
  ...
  public TravelContracts.Response create(TravelContracts.CreateRequest request, UUID managerId) {
    LocalDate start =
        request.startDate() == null ? LocalDate.now().plusDays(30) : request.startDate();
    LocalDate end =
        request.endDate() == null ? start.plusDays(request.durationDays()) : request.endDate();
  ...
  @Transactional
  public TravelContracts.Response update(
      UUID id, TravelContracts.UpdateRequest request, UUID actorId, boolean admin) {
    ...
    travel.updateOffering(
        ...
        price,
        capacity,
        request.status() == null ? travel.getStatus() : request.status());
    return TravelContracts.Response.from(travelRepository.save(travel));
  }
  ...
  private void requireManagerOrAdmin(Travel travel, UUID actorId, boolean admin) {
    if (!admin && (actorId == null || !actorId.equals(travel.getManagerId()))) {
      throw new org.springframework.security.access.AccessDeniedException(
          "Only the organizing manager can modify this travel.");
    }
  }
