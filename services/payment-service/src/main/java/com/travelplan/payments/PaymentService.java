...
public class PaymentService {
    ...
    public PaymentContracts.Response create(PaymentContracts.CreateRequest request) {
        PaymentMethod paymentMethod = new PaymentMethod(request.name().trim(),
                request.provider().trim(), request.enabled());
    ...
    private PaymentMethod requirePaymentMethod(UUID id) {
        return paymentMethodRepository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Payment method " + id + " does not exist."));
    }
}
