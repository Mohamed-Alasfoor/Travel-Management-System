...
public class TransactionService {
  ...
  TransactionService(PaymentTransactionRepository transactions, PaymentMethodRepository methods) {
  ...
  public TransactionContracts.Response charge(UUID traveler, TransactionContracts.ChargeRequest request) {
    return transactions
        ...
        .orElseGet(
            () -> {
              boolean enabled =
                  methods.findAll().stream()
                      .anyMatch(
                          m ->
                              m.isEnabled()
                                  && m.getProvider().equalsIgnoreCase(request.provider()));
              if (!enabled)
                throw new IllegalStateException("Selected payment provider is disabled.");
              String reference =
                  request.provider().toLowerCase(Locale.ROOT) + "_sandbox_" + UUID.randomUUID();
              PaymentTransaction saved =
                  transactions.save(
                      new PaymentTransaction(
                          ...
                          reference));
              return TransactionContracts.Response.from(saved);
            });
  }
